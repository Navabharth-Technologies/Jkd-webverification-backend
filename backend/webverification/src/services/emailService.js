...
exports.sendApprovalEmail = async (toEmail, fullName, password) => {
    ...
    const transporter = nodemailer.createTransport({
        host: (process.env.SMTP_HOST || 'smtp.gmail.com').replace(/^["']|["']$/g, '').trim(),
        ...
        tls: {
            rejectUnauthorized: false // Helps with some Azure/Gmail connection issues
        }
    });

    const senderUser = (process.env.SMTP_USER || 'support@jkdmart.com').replace(/^["']|["']$/g, '').trim();


    const mailOptions = {
        from: `"JKD Mart Admin" <${senderUser}>`,
        to: toEmail,
    ...
    try {
        ...
        console.error(`[Email Service] CRITICAL ERROR sending to ${toEmail}:`);
        console.error(`- Error: ${error.message}`);
        console.error(`- Host: ${process.env.SMTP_HOST || 'smtp.gmail.com'}`);
        console.error(`- Port: ${process.env.SMTP_PORT || '587'}`);
        console.error(`- User: ${process.env.SMTP_USER || 'support@jkdmart.com'}`);
        return false;
    }
};
